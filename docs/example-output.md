...
This document shows sample output from the scanner pipeline
running against `packages/requirements.txt`.

All CVE IDs and package versions shown here correspond to the
intentionally vulnerable packages defined in `packages/requirements.txt`.
...
All detected CVEs are shown. The Security Gate fails because
